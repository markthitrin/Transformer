...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def forward(self, q, k, v, mask, prefix = None, dict = None):
        ...
        if(prefix != None) :
            dict[prefix + ".query"] = query.detach().numpy()
            dict[prefix + ".key"] = key.detach().numpy()
            dict[prefix + ".value"] = value.detach().numpy()
    ...
    def getForward(self, prefix, dict, mask=None) :
        ...
        if(mask == None) :
            ...
            mask = torch.tril(torch.ones((seq, seq), dtype=torch.uint8))
            combined_mask = mask & padding_mask.view(1, -1) & padding_mask.view(-1, 1)

        with torch.no_grad():
            y = self.forward(q, k, v, combined_mask, prefix, dict)
        dict[prefix + ".q"] = q.detach().numpy()
        dict[prefix + ".k"] = k.detach().numpy()
        dict[prefix + ".v"] = v.detach().numpy()
        dict[prefix + ".npd"] =  np.array([num_tokens * 1.0]).astype(np.float32)
    ...
    def getBackward(self, prefix, dict, mask=None) :
        ...
        y = self.forward(q, k, v, combined_mask)
        loss = y.mean()
...
model = MultiHeadAttentionBlock(get_config()["d_model"], get_config()["head"], 0.1)

paramDict = {}
forwardDict = {}
backwardDict = {}

model.getParam("multiheadAtt", paramDict)
save_numpy_dict("multihead_attention_param.npz",paramDict)


for i in range(5) :
    model.getForward("multiheadAtt", forwardDict)
    save_numpy_dict(f"multihead_attention_forward{i}.npz", forwardDict)

for i in range(5):
    model.getBackward("multiheadAtt", backwardDict)
    save_numpy_dict(f"multihead_attention_backward{i}.npz", backwardDict)