...
encoder_blocks = []
for _ in range(N):
    encoder_self_attention_block = MultiHeadAttentionBlock(get_config()["d_model"], 8, 0.1)
    feed_forward_block = FeedForwardBlock(get_config()["d_model"], get_config()["d_ff"], 0.1)
    encoder_block = EncoderBlock(get_config()["d_model"], encoder_self_attention_block, feed_forward_block, 0.1)
    encoder_blocks.append(encoder_block)

model = Encoder(get_config()["d_model"], nn.ModuleList(encoder_blocks))

paramDict = {}
forwardDict = {}
backwardDict = {}

model.getParam("encoder", paramDict)
save_numpy_dict("encoder_param.npz",paramDict)

for i in range(5) :
    model.getForward("encoder", forwardDict)
    save_numpy_dict(f"encoder_forward{i}.npz", forwardDict)

for i in range(5):
    model.getBackward("encoder", backwardDict)
    save_numpy_dict(f"encoder_backward{i}.npz", backwardDict)
