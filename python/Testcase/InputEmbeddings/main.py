...
model = InputEmbeddings(get_config()["d_model"], vocab_size=6)

paramDict = {}
forwardDict = {}
backwardDict = {}

model.getParam("embeddings", paramDict)
save_numpy_dict("embeddings_param.npz",paramDict)


for i in range(5) :
    model.getForward("embeddings", forwardDict)
    save_numpy_dict(f"embeddings_forward{i}.npz", forwardDict)

for i in range(5):
    model.getBackward("embeddings", backwardDict)
    save_numpy_dict(f"embeddings_backward{i}.npz", backwardDict)
