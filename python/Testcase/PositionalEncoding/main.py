...
model = PositionalEncoding(get_config()["d_model"], get_config()["seq"], 0.1)

paramDict = {}
forwardDict = {}
backwardDict = {}

model.getParam("positionalEncoding", paramDict)
save_numpy_dict("positional_encoding_param.npz",paramDict)


for i in range(5) :
    model.getForward("positionalEncoding", forwardDict)
    save_numpy_dict(f"positional_encoding_forward{i}.npz", forwardDict)

for i in range(5):
    model.getBackward("positionalEncoding", backwardDict)
    save_numpy_dict(f"positional_encoding_backward{i}.npz", backwardDict)
